...
| **0.11.x** | **Not required** — Line B may land in 1.1 |
...
| API | **SPEC freeze** | Documented routes and JSON fields in SPEC are stable for 1.x; **additive** fields OK; removals/renames = major (2.0). |
...
- Large new product lines (new Line D).
- Blocking on webhooks/GraphQL.
- Postgres / SPA / multi-writer.
...
3. SPEC marked as 1.0 stability baseline; README points to it.
4. `make release-check` passed; CI + Security workflows green.
5. Packages / ports version fields aligned.

## Checklist

- [ ] Prerequisites from 0.9 / 0.10 verified
